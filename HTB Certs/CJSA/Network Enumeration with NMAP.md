...
It is essential to understand how these services work and to understand the technologies
Attention to detail is the key part in enumeration.It is much more about interacting with the individual services to see what information they provide and what possibilities they offer us.

It is essential to understand how these services work and what syntax they use for effective comms and interaction with different services.
...
When scanning and inspecting we look exactly for these two possibilities.Most information we get comes from misconfigs or neglect of security for the respective services. Misconfigs are either result of ignorance or wrong security mindset. For example, if the administrator only relies on firewall, Group policy objects(GPOs), and continuous updates,it is often not enough to secure network.
...
"Network Mapper(Nmap)" is open-source network analysis and security auditing tool written in C,C++,Python and Lua. It is designed to scan networks and identify which hosts are available on the network using raw packets and services and applications including name and versions of these hosts. Nmap also offers scanning capabilities that can determine if packet filters,firewalls or intrusion detection systems(IDS) are configured as needed.

## UseCases
The tool is one of most used tools by network administrators and IT security specialists. It is used to:
...
```shell-session
0xWAYNE@htb[/htb]$ nmap <scan types> <options> <target>
```
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap -sS localhost
...
## Host Discovery
When performing an internal pentest for company first, we need to get an overview of which systems are online to work with. To actively discover such systems we can use various Nmap host discovery options. There are many options Nmap provides to determine whether our target is alive or not. The most effective host discoveryis to use ICMP echo requests which will be looked.

It is always recommended to store every single scan. This can be later used for comparison,documentation and reporting.After all different tools may produce different results. Therefore it can be beneficial to distinguish which tool produces which results.

#### Scan Network Range
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.0/24 -sn -oA tnet | grep for | cut -d" " -f5
...
#### Scan IP List
During an internal pentest, it is not uncommon for us to be provided with an IP list with hosts we need to test. Nmap also gives us option of working with lists and reading hosts from list instead of manually defining or typing them in.

Such a list could look something like this:
```shell-session
0xWAYNE@htb[/htb]$ cat hosts.lst
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap -sn -oA tnet -iL hosts.lst | grep for | cut -d" " -f5
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap -sn -oA tnet 10.129.2.18 10.129.2.19 10.129.2.20| grep for | cut -d" " -f5
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap -sn -oA tnet 10.129.2.18-20| grep for | cut -d" " -f5
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.18 -sn -oA host 
...
If we disable port scan(-sn), Nmap automatically ping scan with ICMP Echo Requests(-PE). Once such request is sent, we usually expect an ICMP reply if pinging host is alive. The more interesting fact is our previous scans did not do that because before Nmap could send an ICMP echo request, it would send an ARP ping resulting in an ARP reply. We can confirm this with "--packet-trace" option. To ensure that ICMP echo requests are sent, we also define options(-PE) for this.
...
## Host & Port Scanning 
It is essential to understand how the tools we use works and how it performs and processes different functions. The information we need include:

 - Open ports and its services
 - Service versions
 - Information that services provided
 - Operating System
   
There are total of 6 different states for a scanned port we can obtain:
...
On default, Nmap scans the top 1000 TCP ports with SYN scan(-sS). This SYN scan is set only to default when we run it as root because of socket permissions required to create raw TCP packets. 

Otherwise, TCP scan(-sT) is performed by default.This means that if we do not define ports and scanning methods these parameters will be applied automatically.We can define the ports one by one (-p 22,25,80,139,445) by range (-p 22 -445) by top ports (--top - ports=10) from NMAP database that have been signed as most frequent by scanning all ports (-p-) but also by defining a fast port scan which contains top 100 ports(-F).

#### Scanning Top 10 TCP Ports
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 --top-ports=10 
...
We can see that we only received top 10 TCP ports of the target and Nmap dislays their state accordingly.If we trace these packet Nmap sends we will see `RST` flag on `TCP port 21` that our target sends back to us. To have a clear view of SYN  scan, we disable ICMP echo requests(-Pn), DNS resolution(-n) and ARP ping scan(--disable-arp-ping)

#### Nmap - Trace the Packets
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -p 21 --packet-trace -Pn -n --disable-arp-ping
...
### Connect Scan
The Nmap TCP connect Scan(-sT) uses TCP three way handshake to determine if a specific port on a target host is open or closed. The scan sends an SYN packet to target port and waits for response. It is considered open if target port responds with an SYN-ACK packet and closed if it responds with an RST packet.

The `Connect` scan is highly accurate because it completes 3 way TCP handshake allowing us to determine the exact state of port(open,closed or filtered). However it is not the most stealthy. In fact, the connect scan is one of least stealthy techniques as it fully establishes a connection which creates logs on most systems and is easily detected by modern IDS/IPS solutions. That said the connect scan can still be useful in certain situations particularly when accuracy is a priority and goal is to map the network without causing significant disruption to services. Since scan fully establishes a TCP connection it interacts cleanly with services making it less likely to cause service errors or instability compared to more intrusive scans.While it is not the most stealthy method, it is sometimes considered a more "polite" scan because it behaves like a normal client connection, thus having minimal impact on the target services.
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -p 443 --packet-trace --disable-arp-ping -Pn -n --reason -sT 
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -p 139 --packet-trace -n --disable-arp-ping -Pn
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -p 445 --packet-trace -n --disable-arp-ping -Pn
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -F -sU
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -sU -Pn -n --disable-arp-ping --packet-trace -p 137 --reason 
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -sU -Pn -n --disable-arp-ping --packet-trace -p 100 --reason 
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -sU -Pn -n --disable-arp-ping --packet-trace -p 138 --reason 
...
#### Version Scan
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -Pn -n --disable-arp-ping --packet-trace -p 445 --reason  -sV
...
# Saving the Results
### Different Formats
While we run various scans, we should always save results. We can use these later to examine the differences between different scanning methods we have used. Nmap can save results in 3 different formats.
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -p- -oA target
...
#### Normal Output
```shell-session
0xWAYNE@htb[/htb]$ cat target.nmap
...
#### Grepable Output
```shell-session
0xWAYNE@htb[/htb]$ cat target.gnmap
...
#### XML Output
```shell-session
0xWAYNE@htb[/htb]$ cat target.xml
...
```shell-session
0xWAYNE@htb[/htb]$ xsltproc target.xml -o target.html
```
...
It s recommended to perform a quick port scan first which gives a small overview of available of ports. This causes significantly less traffic, which is advantageous for us because otherwise we can be discovered and blocked by security mechanisms. We deal with first and run a port scan in background which show all open ports(-p-). We can use version scan to scan specific ports for services and versions(-sV).

Full port scan takes quite a long time.To view scan we can press [Space bar] during scan which will cause nmap to show us scan status.

```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV --stats-every=5s
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV -v 
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV -Pn -n --disable-arp-ping --packet-trace
...
#### Tcpdump
```shell-session
0xWAYNE@htb[/htb]$ sudo tcpdump -i eth0 host 10.10.14.2 and 10.129.2.28
...
#### Nc
```shell-session
0xWAYNE@htb[/htb]$  nc -nv 10.129.2.28 25
...
![[Pasted image 20250807232632.png]]
...
#### Default Scripts
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap <target> -sC
```

#### Specific Scripts Category
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap <target> --script <category>
```

#### Defined Scripts
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap <target> --script <script-name>,<script-name>,...
```
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -p 25 --script banner,smtp-commands
...
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -p 80 -A
...
#### Nmap - Vuln Category
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.28 -p 80 -sV --script vuln 
...
#### Default Scan
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.0/24 -F
...
#### Optimized RTT
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.0/24 -F --initial-rtt-timeout 50ms --max-rtt-timeout 100ms
...
#### Default Scan
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.0/24 -F | grep "/tcp" | wc -l
...
#### Reduced Retries
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.0/24 -F --max-retries 0 | grep "/tcp" | wc -l
...
#### Default Scan
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.0/24 -F -oN tnet.default
...
#### Optimized Scan
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.0/24 -F -oN tnet.minrate300 --min-rate 300
...
```shell-session
0xWAYNE@htb[/htb]$ cat tnet.default | grep "/tcp" | wc -l
...
```shell-session
0xWAYNE@htb[/htb]$ cat tnet.minrate300 | grep "/tcp" | wc -l
...
#### Insane Scan
```shell-session
0xWAYNE@htb[/htb]$ sudo nmap 10.129.2.0/24 -F -oN tnet.T5 -T 5
...
```shell-session
0xWAYNE@htb[/htb]$ cat tnet.T5 | grep "/tcp" | wc -l
...
### Firewalls
A firewall is a security measure against unauthorised connection attempts from external networks. Every firewall security system is based on software component that monitors network traffic between firewall and incoming data connections and decides how to handle connection based on rules that are set. It checks whether individual network packets are being passed, ignored or blocked. This mechanism is designed to prevent unwanted connections that could be potentially dangerous.

### IDS/IPS
IDS scans the network for potential attacks analyzes them and reports any detected attacks. IPS actively reads the packet and filters them from entering the network. Both are software components but IPS complements IDS by taking extra defensive measure such as blocking the malicious data from entering. The analysis of such attacks is based on pattern matching and signatures. If specific patterns are detected, such as service detection scan, IPS may prevent the pending connection attempts.

### Determine Firewall and Their Rules
We know when a port is shown as filtered. In most cases firewalls have certain rules set to handle specific connections. These packets can either be dropped or rejected. The dropped packets are ignored and no response is returned from host.
...
Nmap's TCP Ack scan(-sA) method is much harder to filter for firewalls and IDS/IPS systems than regular SYN(-sS) or Connect scans(sT) because they only send a TCP packet with only ACK flag. When a port is closed or open, the host must respond with an RST flg.Unlike outgoing connections, all connection attempts(within SYN flag) from external networks are usually blocked by firewalls. However, the packets with ACK flag are often passed by the firewall because the firewall cannot determine whether connection was first established from external network or internal network.

If we look the scan results we can see the results differ
...
```shell-session
0xWAYNE@htb[/htb]$ ncat -nv --source-port 53 10.129.2.28 50000
...
```┌──(dkvv㉿OMEN)-[~/Desktop/HTB]
└─$ sudo nmap -sV 10.129.47.20 -Pn -p80
Starting Nmap 7.95 ( https://nmap.org ) at 2025-08-11 22:22 IST
Nmap scan report for 10.129.47.20 (10.129.47.20)
...
```bash
──(dkvv㉿OMEN)-[~/Desktop/HTB]
└─$ nmap -sV 10.129.135.16 -Pn -p53 -sU       
Starting Nmap 7.95 ( https://nmap.org ) at 2025-08-11 22:51 IST
Nmap scan report for 10.129.135.16 (10.129.135.16)
Host is up (0.33s latency).

PORT   STATE SERVICE VERSION
53/udp open  domain  (unknown banner: HTB{GoTtgUnyze9Psw4vGjcuMpHRp})
1 service unrecognized despite returning data. If you know the service/version, please submit the following fingerprint at https://nmap.org/cgi-bin/submit.cgi?new-service :
SF-Port53-UDP:V=7.95%I=7%D=8/11%Time=689A2692%P=x86_64-pc-linux-gnu%r(DNSV
...
SF:2\0\x01\x006\xee\x80\0\x14\x01K\x0cROOT-SERVERS\x03NET\0\0\0\x02\0\x01\
SF:0\0\x02\0\x01\x006\xee\x80\0\x04\x01D\xc0\?\0\0\x02\0\x01\x006\xee\x80\
SF:0\x04\x01E\xc0\?\0\0\x02\0\x01\x006\xee\x80\0\x04\x01J\xc0\?\0\0\x02\0\
SF:x01\x006\xee\x80\0\x04\x01C\xc0\?\0\0\x02\0\x01\x006\xee\x80\0\x04\x01F
SF:\xc0\?\0\0\x02\0\x01\x006\xee\x80\0\x04\x01G\xc0\?\0\0\x02\0\x01\x006\x
SF:ee\x80\0\x04\x01H\xc0\?\0\0\x02\0\x01\x006\xee\x80\0\x04\x01M\xc0\?\0\0
SF:\x02\0\x01\x006\xee\x80\0\x04\x01B\xc0\?\0\0\x02\0\x01\x006\xee\x80\0\x
SF:04\x01K\xc0\?\0\0\x02\0\x01\x006\xee\x80\0\x04\x01A\xc0\?\0\0\x02\0\x01
SF:\x006\xee\x80\0\x04\x01I\xc0\?");

Service detection performed. Please report any incorrect results at https://nmap.org/submit/ .
Nmap done: 1 IP address (1 host up) scanned in 25.66 
```
...
```Bash
┌──(dkvv㉿OMEN)-[~/Desktop/HTB]
└─$ sudo nmap 10.129.169.50 -sS -sV -Pn -n --disable-arp-ping --source-port 53 -p50000 -vvv
Starting Nmap 7.95 ( https://nmap.org ) at 2025-08-11 23:00 IST
NSE: Loaded 47 scripts for scanning.
Initiating SYN Stealth Scan at 23:00
Scanning 10.129.169.50 [1 port]
Discovered open port 50000/tcp on 10.129.169.50
Completed SYN Stealth Scan at 23:00, 0.24s elapsed (1 total ports)
Initiating Service scan at 23:00
Scanning 1 service on 10.129.169.50
Completed Service scan at 23:00, 5.00s elapsed (1 service on 1 host)
NSE: Script scanning 10.129.169.50.
NSE: Starting runlevel 1 (of 2) scan.
Initiating NSE at 23:00
Stats: 0:00:14 elapsed; 0 hosts completed (1 up), 1 undergoing Script Scan
NSE: Active NSE Script Threads: 1 (1 waiting)
NSE Timing: About 97.78% done; ETC: 23:00 (0:00:00 remaining)
Stats: 0:00:15 elapsed; 0 hosts completed (1 up), 1 undergoing Script Scan
NSE: Active NSE Script Threads: 1 (1 waiting)
NSE Timing: About 97.78% done; ETC: 23:00 (0:00:00 remaining)
Completed NSE at 23:01, 30.01s elapsed
NSE: Starting runlevel 2 (of 2) scan.
Initiating NSE at 23:01
Completed NSE at 23:01, 0.00s elapsed
Nmap scan report for 10.129.169.50
Host is up, received user-set (0.22s latency).
Scanned at 2025-08-11 23:00:35 IST for 36s
...
```Bash
┌──(dkvv㉿OMEN)-[~/Desktop/HTB]
└─$ netcat -nv -p 53 10.129.169.50 50000
(UNKNOWN) [10.129.169.50] 50000 (?) open
